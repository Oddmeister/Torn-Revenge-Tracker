...
GM_xmlhttpRequest({ method: 'GET', url: https://api.torn.com/user/?selections=attacks&key=${API_KEY}, onload(resp) {
try {
const data = JSON.parse(resp.responseText);
let added = 0;
if (data.attacks) for (const id in data.attacks) {
...
if (!log.some(e => e.attackId === entry.attackId)) { log.push(entry); added++; }
}
}
if (added) { saveLog(); showNotification(${added} new event${added>1?'s':''}!); }
console.log('Fetch complete, entries:', log.length);
...
if (e.type === 'Mug') r.mugs++; else if (e.type === 'Hospitalize') r.hosps++;
r.money += e.money; if (e.time > r.last) r.last = e.time;
});
...
Object.assign(panel.style, {
  position: 'fixed', top: isMobile? 'auto':'60px', bottom: isMobile? '60px':'auto',
  right: '20px', width: isMobile? '220px':'260px',
  background: '#1e1e1e', color: '#ddd', padding: '8px', borderRadius: '6px',
  zIndex: 99999, fontFamily: 'Arial, sans-serif', fontSize: '12px', boxShadow: '0 2px 6px rgba(0,0,0,0.7)',
  maxHeight: isMobile? '40%':'none', overflowY: isMobile? 'auto':'visible', display: isMobile? 'none':'block'
});
// Header
panel.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center;">
  ...
  </span></div><hr style="border:none;border-top:1px solid #444;margin:6px 0;">`;
if (!data.length) panel.innerHTML += `<div style="text-align:center;color:#666;">No attackers logged</div>`;
  ...
  const attackBtn = r.id!=='anon'? `<button data-id="${r.id}" class="btn-attack" style="background:#2a2;color:#cfc;border:none;padding:2px 6px;margin-right:4px;border-radius:3px;cursor:pointer;font-size:11px;">Attack</button>`: '';
  const bountyBtn = r.id!=='anon'? `<button data-id="${r.id}" class="btn-bounty" style="background:#2a2;color:#cff;border:none;padding:2px 6px;margin-right:4px;border-radius:3px;cursor:pointer;font-size:11px;">Bounty</button>`: '';
  panel.innerHTML += `<div style="margin-bottom:8px;"><div style="display:flex;justify-content:space-between;"><span><strong>${r.name}</strong> <em style="color:#888;">${r.score.toFixed(1)}</em></span></div>
    <div style="font-size:11px;color:#aaa;">M:${r.mugs} H:${r.hosps} $:${r.money.toLocaleString()} L:${date}</div>
    <div style="background:#333;height:5px;border-radius:3px;overflow:hidden;margin:4px 0;"><div style="width:${Math.floor(r.pct*100)}%;height:100%;background:#6c6;"></div></div>
    <div style="text-align:right;">${attackBtn}${bountyBtn}<button data-id="${r.id}" class="btn-ignore" style="background:none;border:none;color:#f88;cursor:pointer;font-size:11px;">✕</button></div></div>`;
});
document.body.appendChild(panel);
// Toggle button for mobile
if (isMobile) {
  const toggle = document.createElement('button'); toggle.id = 'rev-toggle'; toggle.textContent = '☠️';
  Object.assign(toggle.style, {
    position:'fixed', bottom:'20px', right:'20px', zIndex:99999,
    background:'#1e1e1e', color:'#ddd', border:'none', padding:'8px', borderRadius:'50%', fontSize:'16px', cursor:'pointer', boxShadow:'0 2px 6px rgba(0,0,0,0.7)'
  });
  toggle.onclick = () => panel.style.display = panel.style.display==='none'?'block':'none';
  document.body.appendChild(toggle);
}
// Handlers
panel.querySelector('#clear-all').onclick = () => { if (confirm('Clear all revenge data?')) { localStorage.removeItem(LOG_KEY); log=[]; createUI(); }};
panel.querySelector('#show-ignored').onclick = () => { alert(ignoreList.length? 'Ignored IDs: '+ignoreList.join(', ') : 'No ignored attackers'); };
panel.querySelectorAll('.btn-ignore').forEach(btn=>btn.onclick=e=>{ignoreList.includes(e.target.dataset.id)||ignoreList.push(e.target.dataset.id);saveIgnore();createUI();});
panel.querySelectorAll('.btn-attack').forEach(btn=>btn.onclick=e=>window.open(`https://www.torn.com/attack.php?XID=${e.target.dataset.id}`,'_blank'));
panel.querySelectorAll('.btn-bounty').forEach(btn=>btn.onclick=e=>window.open(`https://www.torn.com/bounty.php?ID=${e.target.dataset.id}`,'_blank'));
  }
